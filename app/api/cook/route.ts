...
const openai = new OpenAI({
    apiKey: "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"
});

export async function POST(req: Request) {
    let { data, context } = await req.json()

    if (!data || !context) return new Response('Missing data or context', { status: 400 })

    let prompt = `The following is a conversation between a Human and an AI assistant expert on data analysis and visualization with perfect javascript syntax.
